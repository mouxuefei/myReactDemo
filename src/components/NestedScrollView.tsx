...
import type { CustomTabBarStyle, DefaultTabBarStyle } from './DefaultTabBar';
...
type Props = {
  ...
  customTabBarStyle?: CustomTabBarStyle; // 自定义 tabbar 样式
  renderTabBar?: () => JSX.Element;
};

const DEBOUNCE_TIME = 300;

class NestedScrollView extends Component<Props> {
  // 保存所有 view 的信息
  private views: VIEWS = {
    // 最上方显示的占位 view
    fakeView: {
  ...
  private diffLists = (nextProps: Props) => {
    const nextLists = nextProps.children
      .filter(child => !!child)
      .map(child => child.key || child.props.titleLabel || '');
  ...
  private initialize = (props: Props) => {
    ...
    const tabs = children.map(child => child.props.titleLabel || '');
  ...
  private panGestrueMove = (index: number, offset: number) => {
  ...
  private handleScrollTabViewScroll = (value: number) => {
    ...
    if (this.views.scrollableTabView.value === value) {
      ...
      this.views.defaultTabbar.scrollValue.setValue(value);
    } else {
      this.views.scrollableTabView.value = value;
      this.views.defaultTabbar.scrollValue.setValue(value);
  ...
  private handleDefaultTabbarRef = (ref: DefaultTabBar) => {
    ...
    if (renderTabBar) {
      ...
      return (
        <DefaultTabBar
          ...
          scrollValue={this.views.defaultTabbar.scrollValue}
  ...
  private renderHeaderSegment = (needShow: boolean = false) => {
    if (needShow) {
      ...
      const tabBarHeight = DefaultTabBar.tabBarHeight(tabBarStyle);
  ...
  // 更新插入头部的view 的 frame
  private updateHeaderFrame = (offsetY: number) => {
  ...
  render() {
    const { tabBarStyle, children } = this.props;
    const tabBarHeight = DefaultTabBar.tabBarHeight(tabBarStyle);
    return (
      <SafeAreaView
