...
export class GoodsList extends Component {
    ...
    return (
      <View>
        {Object.keys(goods).map((item, i) => {
          return (
            <BTouchable key={i} onPress={() => {}}>
              <View style={[styles.typeItem, i == 0 ? styles.active : null]}>
                <Text
                  numberOfLines={2}
                  style={{ fontSize: px2dp(13), color: '#666' }}
                >
                  {item}
                </Text>
              </View>
  ...
  renderList() {
    const { goods, lens } = this.props;
    const _render = (list, index) => {
        ...
        return (
          <View key={obj.key}>
              ...
              <View style={styles.itemWrap}>
                <View style={styles.item}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ fontSize: px2dp(10), color: '#666' }}>
                      {obj.sale}
                    </Text>
                  </View>
                </View>
            ...
            <TouchableOpacity
              ref={obj.key}
              style={styles.addBtn}
              onPress={this.addItem.bind(this, obj)}
            >
              <Image style={styles.icon} />
            </TouchableOpacity>
          </View>
  ...
  render() {
    ...
    const scrollY = this.state.scrollY.interpolate({
      inputRange: [0, headHeight, headHeight],
      outputRange: [0, headHeight, headHeight + 1],
    });
    const style = { flex: 1, flexDirection: 'row', backgroundColor: '#000' };
    ...
    return (
      <View style={style}>
        ...
        <View style={{ flex: 1 }}>
          ...
          >
            <Text style={{ fontSize: px2dp(13), color: '#666' }}>
              {Object.keys(this.props.goods)[0]}
            </Text>
          </View>
        </View>
      </View>
